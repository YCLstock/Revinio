  ...
  var data = [datas['basic_information'],
    ...
    for(p = 0; p < ppl.length; p++){
      ...
      //選取
        ...
        const skill_Width = (skill_Section.getBoundingClientRect().width -3);
        // const Height = Section.getBoundingClientRect().height;           用不到

        console.log("lan ",skill_Width)
...
// console.log(skillsByType);
        ...
        all_skill = skillsByType['language'];                      //取出'技能'類別
        skill_num = skillsByType["language"].length;               //算總數
        ...
        // 創建svg
        const svg = d3.select(".language_content")
        .append("svg")
          .attr("class","language_svg")
        ...
        // 創建多個 <rect> 元素
        const rec = svg
            .selectAll(".language-info") // 使用 selectAll() 來選擇尚不存在的元素
            .data(all_skill) // 綁定資料
            ...
            .on("click", function(event, d) {
                ...
                // const currentColor = d3.select().attr("fill");
        ...
        const labels = svg
          ...
          const sqare = svg
            ...
            .on('click', function(event, d) {
              ...
              const sqareID = d3.selectAll(".language-info").filter((data) => data === d) // 選擇與資料 d 相符的元素
